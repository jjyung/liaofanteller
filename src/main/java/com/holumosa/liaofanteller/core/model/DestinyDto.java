...
import com.holumosa.liaofanteller.core.destiny.config.Config.ConfigType;
...
public class DestinyDto implements Serializable {
  ...
  public DestinyDto(Destiny destiny) {
    ...
    this.starReactionMap =
        destiny.getStarReactionMap().entrySet().stream()
            .collect(
                Collectors.<Entry<StarReaction, Star>, String, Star>toMap(
                    entry -> entry.getKey().getDisplayName(), entry -> entry.getValue()));
