...
public final class Destiny implements Serializable {
  ...
  private static final Set<Star> ABLE_REACTION_STARS_SET =
      new HashSet<Star>(
          Arrays.asList(
  ...
  private Map<MajorStar, StarEnergyLevel> majorStarEnergy =
      new HashMap<MajorStar, StarEnergyLevel>();
  ...
  public Destiny(Config config) {
    this.config = config;
    this.setShadowLight(
        (config.getYearSky().getIndex() % 2 == 0) ? ShadowLight.LIGHT : ShadowLight.SHADOW);
   ...
   *
   * @param shadowLight the new shadow light
   */
   ...
   *
   * @param fiveElement the new five element
   */
   ...
   *
   * @param destinyMaster the new destiny master
   */
   ...
   *
   * @param bodyMaster the new body master
   */
   ...
   *
   * @param pathClockwise the new path clockwise
   */
   ...
   *
   * @param starReaction the star reaction
   ...
   * Gets the star level.
   *
   * @param majorStar the major star
  ...
  public Map<String, Map<MinorStar, Ground>> getRuntimeMinorStarsMap(Lunar lunar) {
    Map<String, Map<MinorStar, Ground>> runtimeMinorStarMap =
        new HashMap<String, Map<MinorStar, Ground>>();
    ...
    Sky yearSky = Sky.getByIndex(CommonUtils.mod(lunar.getYear() % 60 - 4, 60) % 10); // 流年天干
    Sky monthSky =
        Sky.getByIndex(
            (yearSky.getIndex() % 5 + 1) * 2
                + (lunar.getEffectiveMonth() - 1)
                + (lunar.isLeap() && lunar.getDay() > 15 ? 1 : 0)); // 流月天干
    Sky daySky = null; // 流日天干

    // get流日天干
    Optional<Solar> solarOptional =
        LunarUtil.lunar2Solar(lunar.getYear(), lunar.getMonth(), lunar.getDay(), lunar.isLeap());
  ...
  public Map<String, Map<StarReaction, Star>> getRuntimeStarReactionMap(Lunar lunar) {
    ...
    Map<String, Map<StarReaction, Star>> runtimeStarReactionMap =
        new HashMap<String, Map<StarReaction, Star>>();
