...
public final class DestinyBuilder {
   ...
   * @param groundToCellMap the ground to cell map
   */
  public static void setupBasicTempleCells(
      Config config, List<Cell> cells, Map<Ground, Cell> groundToCellMap) {
    ...
    for (int i = 0; i < NUM_CELLS; i++) {
      cells.get(i).setPrevCell(cells.get(CommonUtils.mod(i - 1, NUM_CELLS)));
   ...
   *
   * @param config the config
   * @param templeToCellMap the temple to cell map
   * @param groundToCellMap the ground to cell map
   */
  public static void setupTemples(
      Config config, Map<Temple, Cell> templeToCellMap, Map<Ground, Cell> groundToCellMap) {
    templeToCellMap.put(
        Temple.TEMPLE_BODY,
        groundToCellMap.get(
            getBodyTempleGround(
                config.getDoubleMonthLogicalMonth(), config.getBornTimeGround().getGround())));
    ...
    for (int i = 0; i < Temple.COUNT - 2; i++) {
      templeToCellMap.put(
          Temple.getByIndex(2 + i),
          groundToCellMap.get(
              templeToCellMap.get(Temple.TEMPLE_DESTINY).getGround().shift(-1 - i)));
    }
   ...
   *
   * @param destiny the new up five element
   */
  public static void setupFiveElement(Destiny destiny) {
    ...
    if (skyGroup >= 0
        && skyGroup < Sky.COUNT / 2
        && groundGroup >= 0
        && groundGroup < Ground.COUNT / 2) {
   ...
   *
   * @param fiveElement the five element
  ...
  private static Ground getElementToPersist12BeginGround(FiveElement fiveElement) {
    return new MapWrapper<FiveElement, Ground>(new HashMap<FiveElement, Ground>())
        ...
        .put(FiveElement.FIRE, Ground.getByDisplayName("寅").get())
        .getMap()
        .get(fiveElement);
  }
   ...
   * @param ageStartIndexedCells the age start indexed cells
   */
   ...
   *
   * @param destiny the destiny
   * @param starToCellMap the star to cell map
   * @param majorStarEnergy the major star energy
   */
  public static void setupMajorStars(
      Destiny destiny,
      Map<Star, Cell> starToCellMap,
    ...
    for (MajorStar majorStar : MajorStar.values()) {
      ...
      majorStarEnergy.put(
          majorStar, StarEnergyLevel.getByLevel(majorStar.getStarEnergy(cell.getGround())));
    }
  }
  ...
  public static void setupStars(Destiny destiny, Map<Star, Cell> starToCellMap) {
    Star[] stars =
        new Star[] {
          MinorStar.MINOR_STAR_EMPTY_GROUND, // 地空
    ...
    // 博士12神
    MiniStar[] twelvePreDoctorStars =
        new MiniStar[] {
          MiniStar.MINI_STAR_DOCTOR, // 博士
    ...
    // 歲前12神
    MiniStar[] twelvePreAgeStars =
        new MiniStar[] {
          MiniStar.MINI_STAR_YEAR_GOD, // 太歲
    ...
    // 將前12神
    MiniStar[] twelvePreGeneralStars =
        new MiniStar[] {
          MiniStar.MINI_STAR_GENERAL_STAR, // 將星
   ...
   *
   * @param destiny the new up destiny body master
   */
  public static void setupDestinyBodyMaster(Destiny destiny) {
    destiny.setDestinyMaster(
        new Star[] {
              ...
              MajorStar.MAJOR_STAR_HONEST,
              MajorStar.MAJOR_STAR_MONEY,
              MajorStar.MAJOR_STAR_SOLDIER,
              MajorStar.MAJOR_STAR_MONEY,
              MajorStar.MAJOR_STAR_HONEST,
              MinorStar.MINOR_STAR_SKILL,
              MinorStar.MINOR_STAR_CLOTH,
            ...
            [destiny.getConfig().getYearGround().getIndex()]);

    destiny.setBodyMaster(
        new Star[] {
              MinorStar.MINOR_STAR_FIRE_BURN,
              MajorStar.MAJOR_STAR_HANDPRINT,
   ...
   * Sets the up son dou.
   *
   * @param destiny the new up son dou
   */
   ...
   *
   * @param destiny the new up path clockwise
   */
  public static void setupPathClockwise(Destiny destiny) {
    destiny.setPathClockwise(
        destiny.getShadowLight() == ShadowLight.LIGHT && destiny.getConfig().getSex() == Sex.M
            || destiny.getShadowLight() == ShadowLight.SHADOW
                && destiny.getConfig().getSex() == Sex.F);
  }
   ...
   *
   * @param destiny the destiny
   * @param starReactionMap the star reaction map
   */
   ...
   *
   * @param destiny the new up browser cells
   */
    ...
    for (Cell cell : cells) {
      if (cell.getMajorStars().isEmpty()) {
        ...
        if (cell.getNextICell(4).get().getMajorStars().isEmpty()) {
          cell.setBorrowCells(
              Arrays.asList(
                  new BorrowCell(54.26f, cell.getNextICell(6).get()),
          ...
          cell.setBorrowCells(
              Arrays.asList(
                  new BorrowCell(51f, cell.getNextICell(6).get()),
