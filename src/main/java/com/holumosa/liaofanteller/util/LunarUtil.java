...
public final class LunarUtil {
  ...
  public static final DateTime LUNAR_MIN_DATE =
      new DateTime(1900, 1, 1, 0, 0, 0, DateTimeZone.forID("UTC"));

  /** The Constant LUNAR_MAX_DATE. */
  public static final DateTime LUNAR_MAX_DATE =
      new DateTime(2099, 12, 30, 23, 59, 59, DateTimeZone.forID("UTC"));

  /** The Constant SOLAR_MIN_DATE. */
  public static final DateTime SOLAR_MIN_DATE =
      new DateTime(1900, 1, 31, 0, 0, 0, DateTimeZone.forID("UTC"));

  /** The Constant SOLAR_MAX_DATE. */
  public static final DateTime SOLAR_MAX_DATE =
      new DateTime(2099, 12, 31, 23, 59, 59, DateTimeZone.forID("UTC"));

  /** 大寫月數. */
  private static final String[] CHN_NUMBER = {
    "一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "臘"
  };

  /** 陰曆月數大小. */
  private static final String[] BIG_OR_SMALL = {
  ...
  private static final long[] LUNAR_INFO =
      new long[] {
        ...
        0x0d530, // 2020-2029
        0x05aa0, 0x076a3, 0x096d0, 0x04bdb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520,
        0x0dd45, // 2030-2039
   ...
   * 陰曆轉陽曆
   *
   * <p>輸入的年-1後，循環遍曆獲取該年天數，一直遞減到1900年的己亥年，求出年的總天數。
   *
   * <p>再求出該年的陰曆月日到春節一月一日的天數（這裏就要註意閏月了）。
   *
   * <p>如果傳入的月數大於閏月，則要加上閏月的天數。
   *
   * <p>如果傳入的是閏月，則沒關系，直接加上前面幾個月的天數和自己的日子。
   *
   * <p>最後得到該陰曆日期到1900年1月31日的天數，再換算出公曆Date.
   *
   * @param year the year
   * @param month the month
   * @param day the day
   * @param isLeapMonth 傳入的月是否為閏月
   * @return the optional
   */
  public static Optional<Solar> lunar2Solar(int year, int month, int day, boolean isLeapMonth) {
    ...
    Solar pojo =
        new Solar(solarDate.getYear(), solarDate.getMonthOfYear(), solarDate.getDayOfMonth());
  ...
  public static String animalsYear(int year) {
    final String[] Animals =
        new String[] {"鼠", "牛", "虎", "兔", "龍", "蛇", "馬", "羊", "猴", "雞", "狗", "豬"};
  ...
  public static Optional<Lunar> getPrevLunar(int year, int month, int day, boolean isLeap) {
    ...
    if (solarDate.isPresent()) {
      DateTime calendarRefDay =
          new DateTime(
              solarDate.get().getYear(),
              solarDate.get().getMonth(),
              solarDate.get().getDay(),
              0,
              0,
              0,
              DateTimeZone.forID("UTC"));
      DateTime prevSolarDate = calendarRefDay.plusDays(-1);
      return LunarUtil.solar2Lunar(
          prevSolarDate.getYear(), prevSolarDate.getMonthOfYear(), prevSolarDate.getDayOfMonth());
    }
    return Optional.empty();
  }
  ...
  public static Optional<Lunar> getNextLunar(int year, int month, int day, boolean isLeap) {
    ...
    if (solarDate.isPresent()) {
      ...
      return LunarUtil.solar2Lunar(
          nextSolarDate.getYear(), nextSolarDate.getMonthOfYear(), nextSolarDate.getDayOfMonth());
    }
    return Optional.empty();
  }
  ...
  private static int yearDays(int yyyy) {
    ...
    for (i = 0x8000; i > 0x8; i >>= 1) {
      if ((LUNAR_INFO[yyyy - 1900] & i) != 0) sum += 1;
    }
  ...
  private static int leapDays(int yyyy) {
    if (leapMonth(yyyy) != 0) {
      if ((LUNAR_INFO[yyyy - 1900] & 0x10000) != 0) return 30;
      else return 29;
    } else return 0;
  }
  ...
  private static int monthDays(int y, int m) {
    if ((LUNAR_INFO[y - 1900] & (0x10000 >> m)) == 0) return 29;
    else return 30;
  }
  ...
  private static String getChinaDayString(int day) {
    ...
    if (day == 10) return "初十";
    if (day == 20) return "二十";
    else return chineseTen[day / 10] + CHN_NUMBER[n];
  }
